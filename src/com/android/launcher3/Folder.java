...
public class Folder extends LinearLayout implements DragSource,
	...
	public Folder(Context context, AttributeSet attrs) {
		...
		mMaterialExpandDuration = res
				.getInteger(R.integer.config_materialFolderExpandDuration);
		mMaterialExpandStagger = res
				.getInteger(R.integer.config_materialFolderExpandStagger);
	...
	public boolean onLongClick(View v) {
		...
		if (tag instanceof ShortcutInfo) {
			...
			// DucTMc:
			if (mLauncher.isUnistallAnimationStarted()) {

			} else {
				createBounceAnimations();
				mLauncher.prepareUninstallAnimation();

			}

			/*
			 * // TODO: DucTMc:logic nay co nen mang theo
			 * mLauncher.getWorkspace().beginDragShared(v, this);
			 * 
			 * mCurrentDragInfo = item; mEmptyCell[0] = item.cellX;
			 * mEmptyCell[1] = item.cellY; mCurrentDragView = v;
			 * 
			 * mContent.removeView(mCurrentDragView);
			 * mInfo.remove(mCurrentDragInfo); mDragInProgress = true;
	...
	void bind(FolderInfo info) {
		...
		mFolderIcon.post(new Runnable() {
			public void run() {
				// aZa
				if (getItemCount() <= LAST_ITEM_REMOVE) {
	...
	public void animateOpen() {
		...
		if (!Utilities.isLmpOrAbove()) {
			...
			PropertyValuesHolder tx = PropertyValuesHolder.ofFloat(
					"translationX", transX, 0);
			PropertyValuesHolder ty = PropertyValuesHolder.ofFloat(
					"translationY", transY, 0);

			int rx = (int) Math.max(Math.max(width - getPivotX(), 0),
					getPivotX());
			int ry = (int) Math.max(Math.max(height - getPivotY(), 0),
					getPivotY());
			float radius = (float) Math.sqrt(rx * rx + ry * ry);
			AnimatorSet anim = LauncherAnimUtils.createAnimatorSet();
			Animator reveal = LauncherAnimUtils.createCircularReveal(this,
					(int) getPivotX(), (int) getPivotY(), 0, radius);
			reveal.setDuration(mMaterialExpandDuration);
			reveal.setInterpolator(new LogDecelerateInterpolator(100, 0));

			mContent.setAlpha(0f);
			Animator iconsAlpha = LauncherAnimUtils.ofFloat(mContent, "alpha",
					0f, 1f);
			...
			Animator textAlpha = LauncherAnimUtils.ofFloat(mFolderName,
					"alpha", 0f, 1f);
			textAlpha.setDuration(mMaterialExpandDuration);
			textAlpha.setStartDelay(mMaterialExpandStagger);
			textAlpha.setInterpolator(new AccelerateInterpolator(1.5f));

			Animator drift = LauncherAnimUtils.ofPropertyValuesHolder(this, tx,
					ty);
	...
	public void animateClosed() {
		...
		// mLauncher.stopAnimation(Launcher.ANIMATION_FOLDER_CONTAINER);
	}
	...
	// DucTMc: ham nay set lai so icon cot va hang trong folder open
	...
	private int getContentAreaHeight() {
		...
		int maxContentAreaHeight = grid.availableHeightPx
				...
				- mFolderNameHeight;
		int height = Math
				.min(maxContentAreaHeight, mContent.getDesiredHeight());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int contentAreaWidthSpec = MeasureSpec.makeMeasureSpec(
				getContentAreaWidth(), MeasureSpec.EXACTLY);
		...
		Runnable onCompleteRunnable = new Runnable() {
			...
			public void run() {
				...
				if (getItemCount() <= LAST_ITEM_REMOVE) {
					...
					if (cellLayout != null) {
						// b/12446428 -- sometimes the cell layout has already
						// gone away?
	...
	public void onDrop(DragObject d) {
		...
		// If we are coming from All Apps space, we defer removing the extra
		// empty screen
		// until the folder closes
		if (d.dragSource != mLauncher.getWorkspace()
		...
		if (mIsExternalDrag) {
			...
			// Actually move the item in the database if it was an external
			// drag. Call this
			// before creating the view, so that ShortcutInfo is updated
			// appropriately.
			LauncherModel.addOrMoveItemInDatabase(mLauncher, si, mInfo.id, 0,
					si.cellX, si.cellY);
			...
			CellLayout.LayoutParams lp = (CellLayout.LayoutParams) currentDragView
					.getLayoutParams();
			si.cellX = lp.cellX = mEmptyCell[0];
			si.cellX = lp.cellY = mEmptyCell[1];
			mContent.addViewToCellLayout(currentDragView, -1, (int) si.id, lp,
					true);
		}
		...
		if (d.dragView.hasDrawn()) {
			...
			mLauncher.getDragLayer().animateViewIntoPosition(d.dragView,
					currentDragView, cleanUpRunnable, null);
			setScaleX(scaleX);
			setScaleY(scaleY);
		} else {
		...
		// Temporarily suppress the listener, as we did all the work already
		// here.
		mSuppressOnAdd = true;
		mInfo.add(si);
		mSuppressOnAdd = false;
	...
	// ----------------------aZa------------------------
	private static final int LAST_ITEM_REMOVE = 0;
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (tag instanceof ShortcutInfo && action == MotionEvent.ACTION_DOWN
				&& mLauncher.isUnistallAnimationStarted()) {
			mDownY = event.getRawY();
			mInitilizeX = event.getX();
			mInitilizeY = event.getY();
			mDragRunnable = new DragRunnable(this, v, mLauncher.getWorkspace());
			mHandler.postDelayed(mDragRunnable, 200);
		}

		if (action == MotionEvent.ACTION_MOVE) {
			float eventY = event.getRawY();
			float deltaY = (eventY - mDownY);
			System.out.println("DucTMc: folder deltaY = " + deltaY);
			if (deltaY > 3) {
		...
		if (action == MotionEvent.ACTION_UP) {
			if (mLauncher.isUnistallAnimationStarted()
				...
				float x = event.getX();
				float y = event.getY();
				// System.out.println("DucTMc:huh x = "+ x + "  y= " + y+
				// "  distance= "+ Math.sqrt((x-mInitilizeX)*(x-mInitilizeX) +
				// (y-mInitilizeY)*(y-mInitilizeY)));
				float distance = (float) Math.sqrt((x - mInitilizeX)
						* (x - mInitilizeX) + (y - mInitilizeY)
						* (y - mInitilizeY));
				if (distance < Launcher.TOUCH_SLOP) {
					mHandler.removeCallbacks(mDragRunnable);
					mLauncher.uninstallApplication(v);
				}
	...
	class DragRunnable implements Runnable {
		...
		public DragRunnable(Folder f, View c, Workspace w) {
			folder = f;
			view = c;
			workspace = w;
		}
		...
		public void run() {
			...
			workspace.beginDragShared(view, folder);
			ShortcutInfo item = (ShortcutInfo) tag;
			mCurrentDragInfo = item;
			mEmptyCell[0] = item.cellX;
			mEmptyCell[1] = item.cellY;
	...
	public void createBounceAnimations() {
		// DucTMc
		int itemCount = ((ShortcutAndWidgetContainer) mContent.getChildAt(2))
				.getChildCount();
		ShortcutAndWidgetContainer container = mContent
				.getShortcutsAndWidgets();
		for (int i = 0; i < itemCount; i++) {
			container.getChildAt(i).clearAnimation();
			mLauncher.createBounceAnimation(container.getChildAt(i), 10,
					Launcher.ANIMATION_DESKTOP_CONTAINER);
		}
	}

	public void createBounceAnimations(boolean fromOpen) {
		// DucTMc
		if (fromOpen)
			if (mLauncher.isUnistallAnimationStarted())
				createBounceAnimations();
