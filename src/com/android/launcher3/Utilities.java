...
public final class Utilities {
	 ...
	 */
	static Bitmap createIconBitmap(String packageName, String resourceName,
			...
			if (resources != null) {
				...
				return createIconBitmap(
						resources.getDrawableForDensity(id,
								cache.getFullResIconDpi()), context);
			}
		} catch (Exception e) {
			// Icon not found.
		}
	...
	static Bitmap createIconBitmap(Bitmap icon, Context context) {
		...
		return createIconBitmap(
				new BitmapDrawable(context.getResources(), icon), context);
	}
	 ...
	 */
	public static boolean pointInView(View v, float localX, float localY,
			float slop) {
	...
	static int findDominantColorByHue(Bitmap bitmap, int samples) {
		...
		// where each pixel contributes a score weighted by saturation, value,
		// and alpha.
		...
		for (int y = 0; y < height; y += sampleStride) {
			for (int x = 0; x < width; x += sampleStride) {
				...
				if (hue == bestHue) {
					...
					float newTotal = oldTotal == null ? score : oldTotal
							+ score;
					rgbScores.put(bucket, newTotal);
					if (newTotal > highScore) {
						highScore = newTotal;
						// All the colors in the winning bucket are very
						// similar. Last in wins.
						bestColor = rgb;
					}
				}
			}
	...
	/*
	 * Finds a system apk which had a broadcast receiver listening to a
	 * particular action.
	 * 
	 ...
	 */
	static Pair<String, Resources> findSystemApk(String action,
			PackageManager pm) {
	...
	public static boolean isViewAttachedToWindow(View v) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			...
			// A proxy call which returns null, if the view is not attached to
			// the window.
	 ...
	 * {@link AppWidgetProviderInfo#WIDGET_CATEGORY_SEARCHBOX} provided by the
	 * same package which is set to be global search activity. If widgetCategory
	 * is not supported, or no such widget is found, returns the first widget
	...
	public static AppWidgetProviderInfo getSearchWidgetProvider(Context context) {
		...
		AppWidgetManager appWidgetManager = AppWidgetManager
				.getInstance(context);
		for (AppWidgetProviderInfo info : appWidgetManager
	...
	/************************** Aza ***********************************/
	public static final float ICON_SCALE = 1.75f;

	public static FastBitmapDrawable createUnistallIconDrawable(Bitmap icon,
			boolean unistall, Resources r) {
		FastBitmapDrawable d = new FastBitmapDrawable(icon, unistall, r);
		d.setFilterBitmap(true);
		resizeIconDrawable(d);
		return d;
	}
