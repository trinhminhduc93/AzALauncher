...
public class FolderIcon extends FrameLayout implements FolderListener {
	...
	// Flag whether the folder should open itself when an item is dragged over
	// is enabled.
	...
	static FolderIcon fromXml(int resId, Launcher launcher, ViewGroup group,
		...
		FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) icon.mFolderName
				.getLayoutParams();
		...
		lp = (FrameLayout.LayoutParams) icon.mPreviewBackground
				.getLayoutParams();
		// DucTMc: cho nam cung voi icon binh thuong
//		lp.topMargin = grid.folderBackgroundOffset;
	...
	public static class FolderRingAnimator {
		...
		public FolderRingAnimator(Launcher launcher, FolderIcon folderIcon) {
			...
			if (sStaticValuesDirty) {
				...
				// DucTMc:
				sYPaddingAdded = res.getDimensionPixelSize(R.dimen.folder_y_padding_added);
	...
	public void onDragEnter(Object dragInfo) {
		...
		if (SPRING_LOADING_ENABLED
			...
			// TODO: we currently don't support spring-loading for
			// PendingAddShortcutInfos even
			// though widget-style shortcuts can be added to folders. The issue
			// is that we need
			// to deal with configuration activities which are currently handled
			// in
	 ...
	 * LauncherAppState app = LauncherAppState.getInstance(); DeviceProfile grid
	 * = app.getDynamicGrid().getDeviceProfile();
	 * 
	 * mIntrinsicIconSize = drawableSize; mTotalWidth = totalSize;
	 * 
	 * final int previewSize = mPreviewBackground.getLayoutParams().height;
	 * final int previewPadding = FolderRingAnimator.sPreviewPadding;
	 * 
	 * mAvailableSpaceInPreview = (previewSize - 2 * previewPadding); // cos(45)
	 * = 0.707 + ~= 0.1) = 0.8f int adjustedAvailableSpace = (int)
	 * ((mAvailableSpaceInPreview / 2) * (1 + 0.8f));
	 * 
	 * int unscaledHeight = (int) (mIntrinsicIconSize * (1 +
	 * PERSPECTIVE_SHIFT_FACTOR));
	 * 
	...
	/*
	 * private PreviewItemDrawingParams computePreviewItemDrawingParams(int
	 * index, PreviewItemDrawingParams params) { index = NUM_ITEMS_IN_PREVIEW -
	 * index - 1; float r = (index * 1.0f) / (NUM_ITEMS_IN_PREVIEW - 1); float
	 * scale = (1 - PERSPECTIVE_SCALE_FACTOR * (1 - r));
	 * 
	 * float offset = (1 - r) * mMaxPerspectiveShift; float scaledSize = scale *
	 * mBaselineIconSize; float scaleOffsetCorrection = (1 - scale) *
	 * mBaselineIconSize;
	 * 
	 * // We want to imagine our coordinates from the bottom left, growing up
	 * and to the // right. This is natural for the x-axis, but for the y-axis,
	 * we have to invert things. float transY = mAvailableSpaceInPreview -
	 * (offset + scaledSize + scaleOffsetCorrection) + getPaddingTop(); float
	 * transX = (mAvailableSpaceInPreview - scaledSize) / 2; float totalScale =
	 * mBaselineIconScale * scale; final int overlayAlpha = (int) (80 * (1 -
	 * r));
	 * 
	 * if (params == null) { params = new PreviewItemDrawingParams(transX,
	 * transY, totalScale, overlayAlpha); } else { params.transX = transX;
	 * params.transY = transY; params.scale = totalScale; params.overlayAlpha =
	 * overlayAlpha; } return params; }
	 */
	...
	// DucTMc Chieu y padding nhieu hon x;
	private static int sYPaddingAdded;

	/*
	 * DucTMc: ham tinh toan dau vao cac thong so de ve. Moi folder chi tinh
	 * toan ham nay 1 lan.
	 */
	private void computePreviewDrawingParams(int drawableSize, int totalSize) {
		// Bkav DucTMc: sua de cho ve lai trong truong hop accept Drop
		// if (mIntrinsicIconSize != drawableSize || mTotalWidth != totalSize) {
		LauncherAppState app = LauncherAppState.getInstance();
		DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
		...
		final int previewPadding = FolderRingAnimator.sPreviewPadding;
		final int previewMargin = FolderRingAnimator.sPreviewMargin;

		// mAvailableSpaceInPreview = (previewSize - 2 * previewPadding -
		// previewMargin);
		// DucTMc: khoang trong con lai danh cho cac item
		mAvailableSpaceInPreview = (previewSize - 4 * previewPadding);

		int numOfIcon = (int) Math.sqrt(NUM_ITEMS_IN_PREVIEW);
		int adjustedAvailableSpace = (int) ((mAvailableSpaceInPreview / numOfIcon));

		int unscaledHeight = (int) (mIntrinsicIconSize);
		mBaselineIconScale = (1.0f * adjustedAvailableSpace / unscaledHeight);

		mBaselineIconSize = (int) (mIntrinsicIconSize * mBaselineIconScale);
		mMaxPerspectiveShift = mBaselineIconSize * PERSPECTIVE_SHIFT_FACTOR;
		// Vị tri x,y de bat dau set position cho cac
		// mPreviewOffsetX = /*(mTotalWidth - mAvailableSpaceInPreview-
		// previewMargin) / 2;*/ 62;
		// mPreviewOffsetY = 35;
		// mPreviewOffsetX = 0;
		// mPreviewOffsetY = Y_OFF_SET;
		// Bkav DucTMc: fix again
		mPreviewOffsetX = (mTotalWidth - previewSize) / 2 + previewPadding;
		mPreviewOffsetY = (getMeasuredHeight() - previewSize) / 2;

		// }
	}

	/*
	 * Bkav DucTMc: tinh toan lai position va do scale cua 1 item neu duoc add
	 * vao
	 */
	private PreviewItemDrawingParams computePreviewItemDrawingParams(int index,
		...
		int mNumOfColumns = (int) Math.sqrt(NUM_ITEMS_IN_PREVIEW);
		int rowIndex = (NUM_ITEMS_IN_PREVIEW - index - 1) % mNumOfColumns;
		int columnIndex = (NUM_ITEMS_IN_PREVIEW - index - 1) / mNumOfColumns;

		int internalPosition[] = new int[2];
		// internalPosition[0] = columnIndex * (mBaselineIconSize +
		// FolderRingAnimator.sPreviewPadding);
		// internalPosition[1] = rowIndex * (mBaselineIconSize +
		// FolderRingAnimator.sPreviewMargin);
		// internalPosition[0] = rowIndex * (mBaselineIconSize +
		// ITEM_FOLDER_PADDING );
		// internalPosition[1] = columnIndex * (mBaselineIconSize +
		// ITEM_FOLDER_PADDING);
		internalPosition[0] = rowIndex
				* (mBaselineIconSize + FolderRingAnimator.sPreviewPadding);
		internalPosition[1] = columnIndex
				* (mBaselineIconSize + FolderRingAnimator.sPreviewPadding - sYPaddingAdded);

		float transX = internalPosition[0];
		float transY = internalPosition[1];
		float totalScale = mBaselineIconScale;
		final int overlayAlpha = 80;
